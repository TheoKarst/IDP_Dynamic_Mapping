class MovingEntity {
  ...
  private final float linearSpeed, rotationSpeed;
  ...
  public MovingEntity(Shape shape, float linearSpeed, float rotationSpeed) {
    this.shape = shape;
    ...
    targetAngle = startAngle = shape.angle;
  ...
  public void Update() {
    if(frameCount < endTimeFrame) {
      ...
      float angle = map(frameCount, startTimeFrame, endTimeFrame, startAngle, targetAngle);
      shape.Update(x, y, angle);
    }
    else if(frameCount < endTimeFrame + targetWaitFrames)
      shape.Update(targetX, targetY, targetAngle);
  ...
  private void selectNewTarget() {
    ...
    // Duration to go from the current position to the target:
    float duration = distance / linearSpeed;
    
    startTimeFrame = frameCount;
    endTimeFrame = startTimeFrame + duration;
    targetAngle = random(1) > 0.5f ? startAngle + duration * rotationSpeed : startAngle - duration * rotationSpeed;
  }
