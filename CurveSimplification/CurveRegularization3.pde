ArrayList<PVector> curveRegularization3(ArrayList<PVector> curve, float stepRadius, int minLookAhead, float epsilon) {
    ...
    if(lastExitRadius != -1) {
      ...
      if(lastPivotIndex + 1 < index) {
        ...
        for(int i = lastPivotIndex + 1; i < index; i++) {
          ...
          // If distance > epsilon, we have to keep this point, and thus abort the creation of "nextPivot":
          if(distance > epsilon) {
            lastPivotIndex = i;
            pivot = curve.get(i);
            result.add(pivot);
            abort = true;
            index = i+1;
            //println(" => Abort: Keeping point " + i + " instead.");
            break;
          }
        }
      }
